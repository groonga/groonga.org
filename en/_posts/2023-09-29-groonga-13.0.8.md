...
<!--
フラッグをそもそも使う理由は、圧縮率を下げたいからで、圧縮率が下がるかどうかの判断をするのが同じ数値が連続するかどうか、である。ということで、そもそもの前提を説明する補足を入れましたが、意図がちがったら教えてください。
-->
...
<!--
英語は同じ主語の同じ構文を連続するのを好まないので「このフラグは、圧縮対象の値のバイト間の差分を計算するフラグです。 このフラグは、隣り合うバイトの差分を計算します。」を主語一個で訳してしまいましたが、もし前の文を行うために、具体的には次の分を行う。という意図である場合は文章を変えるので教えてください。　
-->
...
<!--
ここでの「有効です」を「使うことが可能である」と理解して availableと訳していますが、有効に設定できますという話てあれば、　This flag is valid only for `Float`/`Float32`.になります。
-->
...
<!-- 「ここの精度を１バイト落とす」という話がちょっと、Groongaの基礎が分かっていなかったので単純に文字列として訳しています。　Groongaの文書内での精度＝accuracyとなっていてるのでこれに合わせています。類似の単語がPercisionとなっていて、accurencyは真の値に対してどのくらいの精度があるかという意味で、Percisionはそれぞれの値を比較してどのくらい同じにするかみたいな話しです。（コマンドと文書で使っている制度の単語が違うので・・）参考https://manoa.hawaii.edu/exploringourfluidearth/physical/world-ocean/map-distortion/practices-science-precision-vs-accuracy -->
...
<!-- 日本語のほうのコマンドが　１BYTEになっていました野は駄文間違いだと思っています-->
